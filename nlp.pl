...
/* A verb phrase is either a verb or a verb followed
   by a noun phrase.
*/
verb_phrase([X]) :-
	verb(X).
...
/*
**********************************************************
            Parts 3: Conjugations of adverbs
**********************************************************
*/
...
/* this part now includes noun_phrase_better
*/
verb_phrase_better(X) :-
	verb(V),
	noun_phrase_better(NP),
...
/* A conjunction of adverbs is a list of one or
...
/* An adverb list is any permutation of any subset of
   a list of all the adverbs in the lexicon. subseq0 and
   permutation are from the lists library.
*/
adverb_list(AL) :-
...
actions(Actor,Text,As) :-
	findall(Verb,
	      (noun(Actor),followedBy(Actor,Verb,Text),verb(Verb)),
	      As).
